...
app = Flask(__name__)
CORS(app)  # اجازه میده صفحه فرانت (روی گیت‌هاب) به این سرور درخواست بفرسته
...
def policies_to_debug_list(policies):
    out = []
    for p in policies:
        out.append({
            ...
            "financial_percent": p["financial_percent"],
            "bodily_percent": p["bodily_percent"],
            "passenger_percent": p["passenger_percent"],
            "loss_rows": len(p["losses"]),
        })
    return out


@app.route("/api/estelam", methods=["POST"])
def estelam():
    body = request.get_json(force=True, silent=True) or {}
    region = (body.get("region") or "").strip()
    three_digit = (body.get("three_digit") or "").strip()
    letter = (body.get("letter") or "").strip()
    left_two_digit = (body.get("left_two_digit") or "").strip()

    if not region or not three_digit or not letter or not left_two_digit:
        return jsonify({"error": "لطفاً همه فیلدها رو پر کنید"}), 400

    try:
        html = fetch_result_html(region, three_digit, letter, left_two_digit)
        data = extract_policy_json(html)
        if not data:
            return jsonify({"error": "داده‌ای از سایت بیمه پیدا نشد"}), 502

        policies = parse_policies(data)
        if not policies:
            return jsonify({"error": "هیچ بیمه‌نامه ثالثی برای این پلاک پیدا نشد"}), 404

        result = compute_discount(policies)
        result["policies_debug"] = policies_to_debug_list(policies)
